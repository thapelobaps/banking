...
const TransactionsTable = ({ transactions = [] }: TransactionsTableProps) => {
  ...
  return (
    <div className="overflow-hidden rounded-[20px] border border-[#eadfd8] bg-white shadow-sm">
      <div className="flex items-center justify-between gap-3 border-b border-[#eee5df] px-4 py-3">
      ...
      <div className="md:hidden">
        {transactionList.length > 0 ? (
          transactionList.map((transaction) => {
            ...
            return (
              <details key={transaction.id} className="group border-b border-[#f0e8e3] last:border-b-0">
                <summary className="flex min-h-14 cursor-pointer list-none items-center gap-3 px-3 py-3 [&::-webkit-details-marker]:hidden">
                  ...
                  <svg
                    ...
                    stroke="currentColor"
                    strokeWidth="1.8"
                    className="shrink-0 text-[#8a756b] transition-transform duration-200 group-open:rotate-180"
                    aria-hidden="true"
                  >
                    <path d="m6 9 6 6 6-6" />
                  </svg>
